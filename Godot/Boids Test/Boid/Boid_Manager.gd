...
@export_range(1, 100, 1) var Max_Ammo_Capacity : int = 50
...
# All entities are ships with ammo, 
# when a ship has ammo, it can: 
#   - If it's an enemy, seek the target planet and try shoot at it. 
#   - If it's a friendly, seek an enemy boid, and try pursue and shoot it down
# When a ship is out of ammo, it must go back to either the friendly mothership (if it's friendly) or to the enemy mothership (if its an enemy) 
var ALL_ENTITY_AMMOS_comp : PackedByteArray
...
func _ready():
    ...
    ALL_ENTITY_AMMOS_comp = PackedByteArray()
